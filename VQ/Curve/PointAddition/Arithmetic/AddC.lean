/-
Addition of a classical value modulo the secp256k1 prime, in place, over
`VQ.Reversible.constLayout n ws`.

`AddsConstMod m ws n c` asks for one operand field of `n` wires holding `a < m`,
a workspace of `ws` wires starting clear, and the operand replaced by
`(a + c) % m`.  The value added is classical: it is a parameter of the generator,
so it occupies no wire and its bits are known before the circuit exists.  This
classical operand removes the 256 wires that `examples/06-modadd-p` uses to hold
its quantum operand.

Where the two constants a modular addition needs are both classical, one chain
does the work of two.  `examples/06-modadd-p` adds the second operand, records
the carry, then adds `2 ^ k - m` and records that carry, and the flag ends up
holding `a + b ≥ m`.  Only the second of those addends is classical there.  Both
are here, so they fuse: the single constant `2 ^ k + c - m` added to the field
leaves `a + c - m` modulo `2 ^ k`, and its carry out is set exactly when
`a + c ≥ m`, which is the reduction test.  One chain, one `cx` on the flag.

Flipping the flag and adding `m` under its control finishes the value: where no
reduction is due the field holds `2 ^ k + a + c - m` and gains `m`, giving
`a + c`.  Where one is due it already holds `a + c - m` and gains nothing.  The
flag is then cleared by comparing the result against `c`.  Without reduction,
the result is `a + c`, which is at least `c`.  After reduction, it is `a + c - m`,
which is below `c` because `a < m`.  So the carry out of `result + (2 ^ k - c)`
is the flipped flag, the comparison's `cx` cancels it, and no correcting `x` is
needed.

Three chains of `k` positions, so `6 * k` Toffolis against the `8 * k` of
`examples/06-modadd-p`.  The workspace is one carry ancilla, one flag, and one
`k`-bit constant register, so `ws = k + 2`, and no wire holds the classical
value.

At `c = 0` the specification asks for `a % m`, which is `a`, and the generator
emits the identity.  That case is excluded rather than handled because the
closing comparison needs `2 ^ k - c` to fit the `k`-bit register: at `c = 0` it
is `2 ^ k`, `loadX` would load nothing, and the flag would be left dirty.
...
every resource count is uniform in `c`.
...
/-- The constant the first chain adds.  Adding it leaves the field holding
`a + c - m` modulo `2 ^ k`, and its carry out is the reduction test
`a + c ≥ m`. -/
def addend (c : Nat) : Nat := 2 ^ k + c - m

/-- The constant the closing comparison uses.  The carry out of
`result + (2 ^ k - c)` is `result ≥ c`, which is the flipped flag. -/
def cmpc (c : Nat) : Nat := 2 ^ k - c
...
/-- The whole gadget, at a width that can hold the modulus and at a nonzero
classical value.

Read left to right: load `2 ^ k + c - m` and add it, recording the carry, which
is the reduction test.  Unload.  Flip the flag, so it holds `a + c < m`.  Load `m`
under the flag, add it, and unload.  Then load `2 ^ k - c` and compare, which is
the carry out of `result + (2 ^ k - c)`, and that is `result ≥ c`, so it cancels
the flag. -/
def gadget (n c : Nat) : List RGate :=
  loadX (vw n) k (addend c)
...
hold it and the clause of `AddsConstMod` is vacuous.  At `c = 0` the clause asks
for `a % m`, which is `a`.  The circuit is the identity in both cases. -/
def gen (n c : Nat) : RCircuit :=
  { width := n + ws, gates := if k ≤ n ∧ 0 < c then gadget n c else [] }
...
theorem gadget_wf {n : Nat} (hk : k ≤ n) (c : Nat) :
    (gadget n c).all (RGate.wellFormed (n + ws)) = true := by
  have hws : ws = k + 2 := rfl
  have hcv : cw n = n := rfl
  ...
  exact ⟨⟨⟨⟨⟨⟨⟨⟨⟨hlv _, hc1⟩, hlv _⟩, hone⟩, hlc _⟩, hc2⟩, hlc _⟩, hlv _⟩, hc3⟩, hlv _⟩

theorem wf : ∀ n c : Nat, VQ.Reversible.RCircuit.wellFormed (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) = true := by
  intro n c
  show (if k ≤ n ∧ 0 < c then gadget n c else []).all (RGate.wellFormed (n + ws)) = true
  by_cases hk : k ≤ n ∧ 0 < c
  · rw [if_pos hk]; exact gadget_wf hk.1 c
  · rw [if_neg hk]; rfl
...
while a load emits one gate per set bit of its constant.  A tight count of the
loads would depend on the population count of `2 ^ k + c - m` and of `2 ^ k - c`.
The full-width load bounds can exceed that count by at most `6 * k` gates. -/
...
theorem gadget_ccx (n c : Nat) : (gadget n c).countP RGate.isCcx = 6 * k := by
...
theorem gadget_cx (n c : Nat) : (gadget n c).countP RGate.isCx ≤ 14 * k + 2 := by
...
theorem gadget_len (n c : Nat) : (gadget n c).length ≤ 24 * k + 3 := by
  have l1 := loadX_length k (vw n) (addend c)
  have l2 := loadX_length k (vw n) (cmpc c)
  have l3 := loadC_length k (fw n) (vw n) m
  simp only [gadget, blockAdd, blockCmp, List.length_append, achain_length, cchain_length,
    List.length_cons, List.length_nil]
  omega

theorem toffoli_le : ∀ n c : Nat,
    VQ.Circuit.toffoliCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c)) ≤ 1536 := by
  intro n c
  rw [VQ.Reversible.toffoliCount_compile]
  show (if k ≤ n ∧ 0 < c then gadget n c else []).countP RGate.isCcx ≤ 1536
  by_cases hk : k ≤ n ∧ 0 < c
  · rw [if_pos hk, gadget_ccx]; decide
  · rw [if_neg hk]; decide

theorem cnot_le : ∀ n c : Nat,
    VQ.Circuit.cnotCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c)) ≤ 3586 := by
  intro n c
  rw [VQ.Reversible.cnotCount_compile]
  show (if k ≤ n ∧ 0 < c then gadget n c else []).countP RGate.isCx ≤ 3586
  by_cases hk : k ≤ n ∧ 0 < c
  · rw [if_pos hk]
    have := gadget_cx n c
    have hkv : k = 256 := rfl
...
theorem gates_le : ∀ n c : Nat,
    VQ.Circuit.gateCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c)) ≤ 9219 := by
  intro n c
  rw [VQ.Reversible.gateCount_compile]
  show (if k ≤ n ∧ 0 < c then gadget n c else []).length
    + 2 * (if k ≤ n ∧ 0 < c then gadget n c else []).countP RGate.isCcx ≤ 9219
  by_cases hk : k ≤ n ∧ 0 < c
...
theorem gadget_mem (n c : Nat) : ∀ g ∈ gadget n c, ∀ q ∈ g.wires, q ∈ uw n := by
...
theorem wires_le : ∀ n c : Nat,
    VQ.Circuit.usedWires (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c)) ≤ 514 := by
  intro n c
  have hsub : ∀ g ∈ (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c).gates, ∀ q ∈ g.wires, q ∈ uw n := by
    intro g hg
    have hg' : g ∈ (if k ≤ n ∧ 0 < c then gadget n c else []) := hg
    by_cases hk : k ≤ n ∧ 0 < c
    · rw [if_pos hk] at hg'; exact gadget_mem n c g hg'
...
on basis indices.  The lemmas through `VQ.Curve.PointAddition.Arithmetic.AddC.act_cmpV` establish the comparison,
chain, and resource properties.  The cases of `VQ.Curve.PointAddition.Arithmetic.AddC.gadget_act` establish the
gadget's semantics and arithmetic. -/
...
`b1` and `b2` are the successive values of the operand field, `c1` the carry out
of the first chain, which is the reduction test, `f2` the flag it becomes once
flipped, and `c4` the carry of the closing comparison.  The flag returns to zero
because `c4` is `1` exactly where `f2` is, so the comparison needs no correcting
`x`. -/

theorem gadget_act {n c : Nat} (hk : k ≤ n) (hc0 : 0 < c) {i a : Nat}
    (ha : readField i 0 k = a) (hcz : bv i (cw n) = 0) (hfz : bv i (fw n) = 0)
    (hvz : readField i (vw n) k = 0) (ham : a < m) (hcm : c < m) :
    actGates (gadget n c) i = stt n i ((a + c) % m) 0 0 := by
  have hK : 0 < (2 : Nat) ^ k := Nat.two_pow_pos k
  have hmK : m < 2 ^ k := m_lt
  have hm0 : 0 < m := m_pos
  have hWv : addend c = 2 ^ k + c - m := rfl
  have hYv : cmpc c = 2 ^ k - c := rfl
  have haK : a < 2 ^ k := by omega
  have hWK : addend c < 2 ^ k := by omega
  have hYK : cmpc c < 2 ^ k := by omega
  have hi : stt n i a 0 0 = i := stt_id ha hfz hvz
  obtain ⟨b1, hb1⟩ : ∃ x, (addend c + a) % 2 ^ k = x := ⟨_, rfl⟩
  obtain ⟨c1, hc1⟩ : ∃ x, (addend c + a) / 2 ^ k = x := ⟨_, rfl⟩
  have S1 := split_two hK (show addend c + a < 2 * 2 ^ k by omega)
  rw [hb1, hc1] at S1
  ...
  obtain ⟨c4, hc4⟩ : ∃ x, (cmpc c + b2) / 2 ^ k = x := ⟨_, rfl⟩
  have key : b2 = (a + c) % m ∧ (f2 + c4) % 2 = 0 := by
    have S2 := split_two hK (show f2 * m + b1 < 2 * 2 ^ k by omega)
    rw [hb2] at S2
    have S4 := split_two hK (show cmpc c + b2 < 2 * 2 ^ k by omega)
    rw [hc4] at S4
    have SM := split_two hm0 (show a + c < 2 * m by omega)
  ...
  have t1 : actGates (loadX (vw n) k (addend c)) i = stt n i a 0 (addend c) := by
    have h := act_loadV (n := n) i a 0 0 (addend c)
    rw [hi] at h
    rw [h, Nat.zero_mod, Nat.zero_xor]
  have t2 : actGates (blockAdd (vw n) 0 (cw n) (fw n)) (stt n i a 0 (addend c))
      = stt n i b1 c1 (addend c) := by
    have h := act_addV (n := n) hk (b := a) (f := 0) (v := addend c)
      haK (by omega) hWK hcz
    rw [hb1, hc1] at h
    rw [h]
    exact stt_eq rfl (by omega) rfl
  have t3 : actGates (loadX (vw n) k (addend c)) (stt n i b1 c1 (addend c))
      = stt n i b1 c1 0 := by
    rw [act_loadV, Nat.mod_eq_of_lt hWK, Nat.xor_self]
  have t4 : actGates [RGate.x (fw n)] (stt n i b1 c1 0) = stt n i b1 f2 0 := by
    rw [act_flip]
    exact stt_eq rfl (by omega) rfl
  have t5 : actGates (loadC (fw n) (vw n) k m) (stt n i b1 f2 0)
      = stt n i b1 f2 (f2 * m) := by
    rw [act_loadC, Nat.zero_mod, Nat.zero_xor, Nat.mod_eq_of_lt hf2lt]
  have t6 : actGates (achain (vw n) 0 (cw n) [] 0 k) (stt n i b1 f2 (f2 * m))
      = stt n i b2 f2 (f2 * m) := by
    rw [act_addV0 hk hb1K hf2lt hfm hcz, hb2]
  have t7 : actGates (loadC (fw n) (vw n) k m) (stt n i b2 f2 (f2 * m))
      = stt n i b2 f2 0 := by
    rw [act_loadC, Nat.mod_eq_of_lt hfm, Nat.mod_eq_of_lt hf2lt, Nat.xor_self]
  have t8 : actGates (loadX (vw n) k (cmpc c)) (stt n i b2 f2 0)
      = stt n i b2 f2 (cmpc c) := by
    rw [act_loadV, Nat.zero_mod, Nat.zero_xor]
  have t9 : actGates (blockCmp (vw n) 0 (cw n) (fw n)) (stt n i b2 f2 (cmpc c))
      = stt n i b2 ((f2 + c4) % 2) (cmpc c) := by
    rw [act_cmpV hk hb2K hf2lt hYK hcz, hc4]
  have t10 : actGates (loadX (vw n) k (cmpc c))
      (stt n i b2 ((f2 + c4) % 2) (cmpc c)) = stt n i ((a + c) % m) 0 0 := by
    have hkey := key.2
    rw [act_loadV, Nat.mod_eq_of_lt hYK, Nat.xor_self, key.1]
    exact stt_eq rfl (by omega) rfl
  simp only [gadget, actGates_append]
  rw [t1, t2, t3, t4, t5, t6, t7, t8, t9, t10]

/-! ### Correctness claim -/

theorem adds_general : ∀ n c : Nat,
    VQ.Reversible.AddsConstMod VQ.Curve.PointAddition.Arithmetic.AddC.m VQ.Curve.PointAddition.Arithmetic.AddC.ws n c (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) := by
  intro n c a i _ h0 h1 hm ha hc
  have hmK : m < 2 ^ k := m_lt
  have hm0 : 0 < m := m_pos
  have hwsv : ws = k + 2 := rfl
  have e0 : readField i 0 n = a := h0
  show act (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) i = writeField i 0 n ((a + c) % m)
  by_cases hc0 : 0 < c
  · have hk : k ≤ n := k_le hm
    ...
    have hact : act (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) i = actGates (gadget n c) i := by
      show actGates (if k ≤ n ∧ 0 < c then gadget n c else []) i = _
      rw [if_pos (And.intro hk hc0)]
    rw [hact, gadget_act hk hc0 ea ec ef ev ha hc, stt_clean hk ef ev]
    exact write_widen hk (by have := Nat.mod_lt (a + c) hm0; omega) (by rw [e0]; omega)
  · have hval : (a + c) % m = a := by
      rw [show c = 0 by omega, Nat.add_zero]
      exact Nat.mod_eq_of_lt ha
    have hne : ¬ (k ≤ n ∧ 0 < c) := fun h => hc0 h.2
    have hact : act (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) i = i := by
      show actGates (if k ≤ n ∧ 0 < c then gadget n c else []) i = i
      rw [if_neg hne]
      rfl
    have hw : writeField i 0 n a = i := by rw [← e0]; exact writeField_read i 0 n
    rw [hact, hval, hw]

/-- Constant addition with the modulus and workspace specialized to their fixed values. -/
theorem adds : ∀ n c : Nat,
    VQ.Reversible.AddsConstMod
      115792089237316195423570985008687907853269984665640564039457584007908834671663
      258 n c (VQ.Curve.PointAddition.Arithmetic.AddC.gen n c) := adds_general
