...
/-!
A squaring circuit modulo the secp256k1 prime, over `VQ.Reversible.unaryLayout`.

This is `examples/07-modmul` with one operand instead of two.  The multiplicand
and the multiplier are the same field, so the round for bit `t` gates the input
field by wire `t` of the input field.  At the one position where the control
coincides with the source the Toffoli would be `ccx t t d`, which
`RGate.wellFormed` rejects: it requires the three wires of a Toffoli to be
distinct, and every gate list here has to be well formed at the declared width.
A CNOT stands in and computes the same bit, since `a_t ∧ a_t = a_t`.
`VQ.Curve.PointAddition.Arithmetic.Sq.scopy` is the resulting gating routine and is the only structural difference
from the multiplier.  Everything else is 07-modmul with the workspace moved down
one field, from `3 n` to `2 n`, and the output field moved from field two to field
one.

The saving is one Toffoli per gating call.  There are two calls per round, `k`
rounds, and the forward pass appears twice in `VQ.Curve.PointAddition.Arithmetic.Sq.body`, so this construction
costs `4 k` fewer Toffolis and `4 k` more CNOTs than the multiplier, on `k` fewer
wires.  Copying the input into a workspace register and invoking the multiplier
unchanged would cost `k` more wires than the multiplier alone and `2 k` CNOTs for
the copy, and would keep the multiplier's Toffoli count.

## Modulus, widths, and wire assignment

The modulus is fixed and the width `n` is the family
parameter.  The input the specification covers is below the modulus, so only its
low `k` bits carry information, where `k` is the bit length of the modulus.  The
circuit is a gadget of fixed size placed at width-dependent offsets, and every
resource count is a constant.
...
is then `A + dkp` by definition.  `kp` also carries the Toffoli count of one
gating call, which is one less than the number of positions. -/
...
def sReg (n : Nat) : Nat := 2 * n

/-- The product accumulator, `dk` wires. -/
def pReg (n : Nat) : Nat := 2 * n + sk

/-- The register the classical constants are loaded into, `dk` wires. -/
def vReg (n : Nat) : Nat := 2 * n + sk + dk

/-- The quotient bits of the reduction, `k` wires. -/
def qReg (n : Nat) : Nat := 2 * n + sk + dk + dk

/-- The carry-in ancilla of every chain. -/
def cReg (n : Nat) : Nat := 2 * n + sk + dk + dk + k

theorem e_sk : sk = k + 1 := rfl
theorem e_dk : dk = 2 * k := rfl
theorem e_ws : ws = 6 * k + 2 := rfl
theorem e_s (n : Nat) : sReg n = 2 * n := rfl
theorem e_p (n : Nat) : pReg n = 2 * n + sk := rfl
theorem e_v (n : Nat) : vReg n = 2 * n + sk + dk := rfl
theorem e_q (n : Nat) : qReg n = 2 * n + sk + dk + dk := rfl
theorem e_c (n : Nat) : cReg n = 2 * n + sk + dk + dk + k := rfl
...
Modular squaring is a schoolbook product followed by a reduction, and both halves
are uncomputed at the end.  The product accumulator holds `a * a` in full
`2 k`-bit precision, so the round for bit `t` of the input adds the input into the
accumulator at bit offset `t` rather than doubling the accumulator.  A doubling
would have to move the accumulator's bits, which uses a wire permutation.  A
shifted addend uses the same gates at a different offset.
...
The residue is copied out into the output field and the forward pass is then run
backwards, which restores every workspace wire and the input.  The reversed list
undoes the forward one because every gate here is an involution, which is
`VQ.Curve.PointAddition.Arithmetic.Sq.revUndo`. -/
...
/-- The same under the control of wire `ctl`, one Toffoli per position, except
where the source wire is the control itself.  There the Toffoli would name one
wire twice, and `RGate.wellFormed` requires the wires of a multi-wire gate to be
distinct, so a CNOT stands in.  It computes the same bit, since a bit conjoined
with itself is itself.

The multiplier in `examples/07-modmul` controls one operand field with a bit
from a disjoint operand field.  Squaring controls the input field with one of
its own bits, producing one coincident control-and-source position per call.
At that position, a CNOT implements the required self-controlled bit copy. -/
def scopy (ctl : Nat) : Nat → Nat → Nat → List RGate
  | _, _, 0 => []
  | A, D, p + 1 =>
      (if A = ctl then RGate.cx ctl D else RGate.ccx ctl A D)
        :: scopy ctl (A + 1) (D + 1) p

/-- The recursive step of `VQ.Curve.PointAddition.Arithmetic.Sq.scopy`, as a rewrite. -/
theorem scopy_cons (ctl A D p : Nat) :
    scopy ctl A D (p + 1)
      = (if A = ctl then RGate.cx ctl D else RGate.ccx ctl A D)
        :: scopy ctl (A + 1) (D + 1) p := rfl

/-- One round of the product: gate the input by bit `t` of the input, add it into
the accumulator at offset `t`, and clear the gated copy. -/
def mround (n t : Nat) : List RGate :=
  scopy t 0 (sReg n) k
    ++ achain (sReg n) (pReg n + t) (cReg n) [] 0 sk
    ++ scopy t 0 (sReg n) k

/-- The rounds for bits `0` through `t - 1` of the input, in that order. -/
...
/-- The whole gadget: compute, copy the residue into the output field, and
uncompute. -/
def body (n : Nat) : List RGate :=
  fwd n ++ pcopy (pReg n) n k ++ (fwd n).reverse
...
def gen (n : Nat) : RCircuit :=
  { width := 2 * n + ws, gates := if k ≤ n then body n else [] }
...
/-- The control may lie inside the source field.  The degenerate position is a
CNOT and requires distinct control and target wires. -/
theorem scopy_wf {w ctl : Nat} (hctl : ctl < w) : ∀ (p A D : Nat), A + p ≤ w → D + p ≤ w →
    (ctl < D ∨ D + p ≤ ctl) → (A + p ≤ D ∨ D + p ≤ A) →
    (scopy ctl A D p).all (RGate.wellFormed w) = true := by
  intro p
  induction p with
  | zero => intro A D _ _ _ _; rfl
  | succ p ih =>
    intro A D hA hD hcD hAD
    simp only [scopy, List.all_cons, Bool.and_eq_true]
    refine ⟨?_, ih (A + 1) (D + 1) (by omega) (by omega) (by omega) (by omega)⟩
    have hDw : D < w := by omega
    have hcD' : ctl ≠ D := by omega
    by_cases hA' : A = ctl
    · rw [if_pos hA']
      simp [RGate.wellFormed, hctl, hDw, hcD']
    · rw [if_neg hA']
      have hAw : A < w := by omega
      have hcA : ctl ≠ A := fun h => hA' h.symm
      have hAD' : A ≠ D := by omega
      simp [RGate.wellFormed, hctl, hAw, hDw, hcA, hcD', hAD']

theorem mround_wf {n : Nat} (hk : k ≤ n) {t : Nat} (ht : t < k) :
    (mround n t).all (RGate.wellFormed (2 * n + ws)) = true := by
  have h1 := e_sk
  have h2 := e_dk
  have h3 := e_ws
  have h4 := e_s n
  ...
  have h7 := e_q n
  have h8 := e_c n
  have hcp : (scopy t 0 (sReg n) k).all (RGate.wellFormed (2 * n + ws)) = true :=
    scopy_wf (by omega) k 0 (sReg n) (by omega) (by omega) (by omega) (by omega)
...
theorem mloop_wf {n : Nat} (hk : k ≤ n) : ∀ t, t ≤ k →
    (mloop n t).all (RGate.wellFormed (2 * n + ws)) = true := by
  intro t
  induction t with
  | zero => intro _; rfl
  | succ t ih =>
...
theorem rstep_wf {n : Nat} (_hk : k ≤ n) {t : Nat} (ht : t < k) :
    (rstep n t).all (RGate.wellFormed (2 * n + ws)) = true := by
  have h1 := e_sk
  have h2 := e_dk
  have h3 := e_ws
  have h4 := e_s n
  ...
  have hlx : ∀ v, (loadX (vReg n) dk v).all (RGate.wellFormed (2 * n + ws)) = true :=
  ...
  have hx : ([RGate.x (qReg n + t)]).all (RGate.wellFormed (2 * n + ws)) = true :=
    x_wf (by omega)
...
theorem rloop_wf {n : Nat} (hk : k ≤ n) : ∀ t, t ≤ k →
    (rloop n t).all (RGate.wellFormed (2 * n + ws)) = true := by
  intro t
  induction t with
  | zero => intro _; rfl
  | succ t ih =>
...
theorem fwd_wf {n : Nat} (hk : k ≤ n) :
    (fwd n).all (RGate.wellFormed (2 * n + ws)) = true := by
...
theorem body_wf {n : Nat} (hk : k ≤ n) :
    (body n).all (RGate.wellFormed (2 * n + ws)) = true := by
  have h1 := e_sk
  have h2 := e_dk
  have h3 := e_ws
  have h5 := e_p n
  have hf := fwd_wf hk
  have hcp : (pcopy (pReg n) n k).all (RGate.wellFormed (2 * n + ws)) = true :=
    pcopy_wf k (pReg n) n (by omega) (by omega) (Or.inr (by omega))
  simp only [body, List.all_append, Bool.and_eq_true]
  exact ⟨⟨hf, hcp⟩, all_reverse hf⟩

theorem wf : ∀ n : Nat, VQ.Reversible.RCircuit.wellFormed (VQ.Curve.PointAddition.Arithmetic.Sq.gen n) = true := by
  intro n
  show (if k ≤ n then body n else []).all (RGate.wellFormed (2 * n + ws)) = true
  by_cases hk : k ≤ n
...
theorem scopy_length : ∀ (p ctl A D : Nat), (scopy ctl A D p).length = p := by
  intro p
  induction p with
  | zero => intro ctl A D; rfl
  | succ p ih =>
    intro ctl A D
    rw [scopy_cons, List.length_cons, ih ctl (A + 1) (D + 1)]

/-! The head of a gating call is an `if`, so each count induction has to case on
it and then step over a known head.  These four are the four steps: the name says
which count is being taken and which gate heads the list. -/

theorem isCcx_cons_ccx (a b c : Nat) (l : List RGate) :
    (RGate.ccx a b c :: l).countP RGate.isCcx = l.countP RGate.isCcx + 1 := by
  simp [List.countP_cons, RGate.isCcx]

theorem isCcx_cons_cx (a b : Nat) (l : List RGate) :
    (RGate.cx a b :: l).countP RGate.isCcx = l.countP RGate.isCcx := by
  simp [RGate.isCcx]

theorem isCx_cons_ccx (a b c : Nat) (l : List RGate) :
    (RGate.ccx a b c :: l).countP RGate.isCx = l.countP RGate.isCx := by
  simp [RGate.isCx]

theorem isCx_cons_cx (a b : Nat) (l : List RGate) :
    (RGate.cx a b :: l).countP RGate.isCx = l.countP RGate.isCx + 1 := by
  simp [List.countP_cons, RGate.isCx]

/-- Every position of a gating call is a Toffoli when the control lies outside the
source field. -/
theorem scopy_ccx_out {ctl : Nat} : ∀ (p A D : Nat), (ctl < A ∨ A + p ≤ ctl) →
    (scopy ctl A D p).countP RGate.isCcx = p := by
  intro p
  induction p with
  | zero => intro A D _; rfl
  | succ p ih =>
    intro A D h
    have hrec := ih (A + 1) (D + 1) (show ctl < A + 1 ∨ A + 1 + p ≤ ctl by omega)
    have hA : ¬ (A = ctl) := by omega
    rw [scopy_cons, if_neg hA, isCcx_cons_ccx]
    omega

theorem scopy_cx_out {ctl : Nat} : ∀ (p A D : Nat), (ctl < A ∨ A + p ≤ ctl) →
    (scopy ctl A D p).countP RGate.isCx = 0 := by
  intro p
  induction p with
  | zero => intro A D _; rfl
  | succ p ih =>
    ...
    have hrec := ih (A + 1) (D + 1) (show ctl < A + 1 ∨ A + 1 + p ≤ ctl by omega)
    have hA : ¬ (A = ctl) := by omega
    rw [scopy_cons, if_neg hA, isCx_cons_ccx]
    omega

/-- One position of a gating call is a CNOT and the rest are Toffolis when the
control lies inside the source field.  Stated as a successor rather than with a
truncated subtraction, so that the loop counts stay inside `omega`'s
fragment. -/
theorem scopy_ccx_in {ctl : Nat} : ∀ (p A D : Nat), A ≤ ctl → ctl < A + p →
    (scopy ctl A D p).countP RGate.isCcx + 1 = p := by
  intro p
  induction p with
  | zero => intro A D hlo hhi; omega
  | succ p ih =>
    intro A D hlo hhi
    by_cases hA : A = ctl
    · have hrec := scopy_ccx_out p (A + 1) (D + 1) (Or.inl (show ctl < A + 1 by omega))
      rw [scopy_cons, if_pos hA, isCcx_cons_cx]
      omega
    · have hrec := ih (A + 1) (D + 1) (by omega) (by omega)
      rw [scopy_cons, if_neg hA, isCcx_cons_ccx]
      omega

theorem scopy_cx_in {ctl : Nat} : ∀ (p A D : Nat), A ≤ ctl → ctl < A + p →
    (scopy ctl A D p).countP RGate.isCx = 1 := by
  intro p
  induction p with
  | zero => intro A D hlo hhi; omega
  | succ p ih =>
    intro A D hlo hhi
    by_cases hA : A = ctl
    · have hrec := scopy_cx_out p (A + 1) (D + 1) (Or.inl (show ctl < A + 1 by omega))
      rw [scopy_cons, if_pos hA, isCx_cons_cx]
      omega
    · have hrec := ih (A + 1) (D + 1) (by omega) (by omega)
      rw [scopy_cons, if_neg hA, isCx_cons_ccx]
      omega

/-- The three counts of one product round.  The gating calls read the field they
are controlled by, so each contributes `kp` Toffolis and one CNOT rather than `k`
Toffolis. -/
theorem mround_counts (n : Nat) {t : Nat} (ht : t < k) :
    (mround n t).length = 2 * k + 6 * sk ∧
      (mround n t).countP RGate.isCcx = 2 * kp + 2 * sk ∧
      (mround n t).countP RGate.isCx = 4 * sk + 2 := by
  have hkk : k = kp + 1 := rfl
  have hcc := scopy_ccx_in (ctl := t) k 0 (sReg n) (Nat.zero_le t) (by omega)
  have hcx := scopy_cx_in (ctl := t) k 0 (sReg n) (Nat.zero_le t) (by omega)
  have hn : ([] : List RGate).countP RGate.isCcx = 0 := rfl
  have hn' : ([] : List RGate).countP RGate.isCx = 0 := rfl
  refine ⟨?_, ?_, ?_⟩
  · simp only [mround, List.length_append, scopy_length, achain_length, List.length_nil]
    omega
  · simp only [mround, List.countP_append, achain_ccx, hn]
    omega
  · simp only [mround, List.countP_append, achain_cx, hn']
    omega

theorem mloop_counts (n : Nat) : ∀ t, t ≤ k →
    (mloop n t).length = t * (2 * k + 6 * sk) ∧
      (mloop n t).countP RGate.isCcx = t * (2 * kp + 2 * sk) ∧
      (mloop n t).countP RGate.isCx = t * (4 * sk + 2) := by
  intro t
  induction t with
  ...
  | succ t ih =>
    ...
    obtain ⟨r1, r2, r3⟩ := mround_counts n (show t < k by omega)
    have s1 : (t + 1) * (2 * k + 6 * sk) = t * (2 * k + 6 * sk) + (2 * k + 6 * sk) :=
      Nat.succ_mul t _
    have s2 : (t + 1) * (2 * kp + 2 * sk) = t * (2 * kp + 2 * sk) + (2 * kp + 2 * sk) :=
      Nat.succ_mul t _
    have s3 : (t + 1) * (4 * sk + 2) = t * (4 * sk + 2) + (4 * sk + 2) := Nat.succ_mul t _
    refine ⟨?_, ?_, ?_⟩
...
theorem fwd_counts (n : Nat) :
    (fwd n).length ≤ k * (2 * k + 6 * sk) + k * (16 * dk + 3) ∧
      (fwd n).countP RGate.isCcx = k * (2 * kp + 2 * sk) + k * (4 * dk) ∧
      (fwd n).countP RGate.isCx ≤ k * (4 * sk + 2) + k * (10 * dk + 1) := by
  obtain ⟨m1, m2, m3⟩ := mloop_counts n k (Nat.le_refl _)
...
theorem body_counts (n : Nat) :
    (body n).length ≤ 2 * (k * (2 * k + 6 * sk) + k * (16 * dk + 3)) + k ∧
      (body n).countP RGate.isCcx = 2 * (k * (2 * kp + 2 * sk) + k * (4 * dk)) ∧
      (body n).countP RGate.isCx ≤ 2 * (k * (4 * sk + 2) + k * (10 * dk + 1)) + k := by
  ...
  have hr3 : (fwd n).reverse.countP RGate.isCx = (fwd n).countP RGate.isCx :=
    List.countP_reverse
  have p1 := pcopy_length k (pReg n) n
  have p2 := pcopy_ccx k (pReg n) n
  have p3 := pcopy_cx k (pReg n) n
  refine ⟨?_, ?_, ?_⟩
...
theorem toffoli_le : ∀ n : Nat,
    VQ.Circuit.toffoliCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)) ≤ 1572864 := by
  intro n
  rw [VQ.Reversible.toffoliCount_compile]
  show (if k ≤ n then body n else []).countP RGate.isCcx ≤ 1572864
  by_cases hk : k ≤ n
  · rw [if_pos hk]
...
theorem cnot_le : ∀ n : Nat,
    VQ.Circuit.cnotCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)) ≤ 3149568 := by
  intro n
  rw [VQ.Reversible.cnotCount_compile]
  show (if k ≤ n then body n else []).countP RGate.isCx ≤ 3149568
  by_cases hk : k ≤ n
  · rw [if_pos hk]
...
`2 * k + ws` wires has at most that many entries. -/
...
/-- The wires the gadget names: the low `k` of the input field, the low `k` of the
output field, and the whole workspace. -/
def uw (n : Nat) : List Nat :=
  List.range k ++ (List.range k).map (fun j => n + j)
    ++ (List.range ws).map (fun j => 2 * n + j)

theorem uw_length (n : Nat) : (uw n).length = 2 * k + ws := by
...
theorem mem_uw_p {n j : Nat} (h : j < k) : n + j ∈ uw n := by
  simp only [uw, List.mem_append, List.mem_map, List.mem_range]
  exact Or.inl (Or.inr ⟨j, h, rfl⟩)

theorem mem_uw_w {n j : Nat} (h : j < ws) : 2 * n + j ∈ uw n := by
  simp only [uw, List.mem_append, List.mem_map, List.mem_range]
...
theorem scopy_mem {P : Nat → Prop} {ctl : Nat} (hctl : P ctl) :
    ∀ (p A D : Nat), (∀ j, j < p → P (A + j)) → (∀ j, j < p → P (D + j)) →
      ∀ g ∈ scopy ctl A D p, ∀ q ∈ g.wires, P q := by
  intro p
  induction p with
  | zero => intro A D _ _ g hg; simp [scopy] at hg
  | succ p ih =>
    intro A D hA hD g hg q hq
    have hA0 : P A := by have := hA 0 (by omega); simpa using this
    have hD0 : P D := by have := hD 0 (by omega); simpa using this
    have htail : ∀ g ∈ scopy ctl (A + 1) (D + 1) p, ∀ q ∈ g.wires, P q := by
      refine ih (A + 1) (D + 1) (fun j hj => ?_) (fun j hj => ?_)
      · have := hA (j + 1) (by omega)
        have he : A + 1 + j = A + (j + 1) := by omega
        rw [he]; exact this
      · have := hD (j + 1) (by omega)
    ...
    rw [scopy_cons] at hg
    by_cases hAc : A = ctl
    · rw [if_pos hAc, List.mem_cons] at hg
      rcases hg with rfl | hg
      · simp only [RGate.wires, List.mem_cons, List.not_mem_nil, or_false] at hq
        rcases hq with rfl | rfl
        · exact hctl
        · exact hD0
      · exact htail g hg q hq
    · rw [if_neg hAc, List.mem_cons] at hg
      rcases hg with rfl | hg
      · simp only [RGate.wires, List.mem_cons, List.not_mem_nil, or_false] at hq
...
theorem mround_mem {n : Nat} {P : Nat → Prop} (_hk : k ≤ n) {t : Nat} (ht : t < k)
  ...
  have hctl : P t := hA t ht
  have hS : ∀ j, j < sk → P (sReg n + j) := by
    intro j hj
    have he : sReg n + j = 2 * n + j := rfl
    rw [he]; exact hW j (by omega)
  have hP : ∀ j, j < sk → P (pReg n + t + j) := by
    intro j hj
    have he : pReg n + t + j = 2 * n + (sk + t + j) := by rw [e_p]; omega
    rw [he]; exact hW _ (by omega)
  have hc : P (cReg n) := by
    have he : cReg n = 2 * n + (sk + dk + dk + k) := by rw [e_c]; omega
    rw [he]; exact hW _ (by omega)
  have hcp : ∀ g ∈ scopy t 0 (sReg n) k, ∀ q ∈ g.wires, P q :=
    scopy_mem hctl k 0 (sReg n) hA' (fun j hj => hS j (by omega))
...
theorem mloop_mem {n : Nat} {P : Nat → Prop} (hk : k ≤ n)
  ...
  | succ t ih =>
    ...
    · exact mround_mem hk (by omega) hA hW g h
...
theorem rstep_mem {n : Nat} {P : Nat → Prop} (_hk : k ≤ n) {t : Nat} (ht : t < k)
  ...
  have hV : ∀ j, j < dk → P (vReg n + j) := by
    intro j hj
    have he : vReg n + j = 2 * n + (sk + dk + j) := by rw [e_v]; omega
    rw [he]; exact hW _ (by omega)
  have hP : ∀ j, j < dk → P (pReg n + j) := by
    intro j hj
    have he : pReg n + j = 2 * n + (sk + j) := by rw [e_p]; omega
    rw [he]; exact hW _ (by omega)
  have hq : P (qReg n + t) := by
    have he : qReg n + t = 2 * n + (sk + dk + dk + t) := by rw [e_q]; omega
    rw [he]; exact hW _ (by omega)
  have hc : P (cReg n) := by
    have he : cReg n = 2 * n + (sk + dk + dk + k) := by rw [e_c]; omega
    rw [he]; exact hW _ (by omega)
...
theorem fwd_mem {n : Nat} {P : Nat → Prop} (hk : k ≤ n)
    (hA : ∀ j, j < k → P j)
    (hW : ∀ j, j < ws → P (2 * n + j)) : ∀ g ∈ fwd n, ∀ q ∈ g.wires, P q := by
  intro g hg
  rcases List.mem_append.mp hg with h | h
  · exact mloop_mem hk hA hW k (Nat.le_refl _) g h
  · exact rloop_mem hk hW k (Nat.le_refl _) g h

theorem body_mem {n : Nat} {P : Nat → Prop} (hk : k ≤ n)
    (hA : ∀ j, j < k → P j)
    (hD : ∀ j, j < k → P (n + j)) (hW : ∀ j, j < ws → P (2 * n + j)) :
    ∀ g ∈ body n, ∀ q ∈ g.wires, P q := by
  have h1 := e_sk
  have h3 := e_ws
  have hcp : ∀ g ∈ pcopy (pReg n) n k, ∀ q ∈ g.wires, P q := by
    refine pcopy_mem k (pReg n) n (fun j hj => ?_) (fun j hj => hD j hj)
    have he : pReg n + j = 2 * n + (sk + j) := by rw [e_p]; omega
    rw [he]; exact hW _ (by omega)
  intro g hg
  rcases List.mem_append.mp hg with h | h
  · rcases List.mem_append.mp h with h' | h'
    · exact fwd_mem hk hA hW g h'
    · exact hcp g h'
  · exact fwd_mem hk hA hW g (List.mem_reverse.mp h)
...
theorem body_uw {n : Nat} (hk : k ≤ n) : ∀ g ∈ body n, ∀ q ∈ g.wires, q ∈ uw n :=
  body_mem (P := fun q => q ∈ uw n) hk (fun _ hj => mem_uw_a hj)
    (fun _ hj => mem_uw_p hj) (fun _ hj => mem_uw_w hj)

/-- The forward pass avoids every wire of the output field.  The residue can be
copied out between the forward pass and its reverse.  The pass reads the input
field twice and leaves field one unchanged. -/
theorem fwd_out {n : Nat} (hk : k ≤ n) :
    ∀ g ∈ fwd n, ∀ q ∈ g.wires, q < n ∨ n + k ≤ q := by
  have hkp := k_pos
  exact fwd_mem (P := fun q => q < n ∨ n + k ≤ q) hk
    (fun j hj => Or.inl (by omega))
...
theorem wires_le : ∀ n : Nat,
    VQ.Circuit.usedWires (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)) ≤ 2050 := by
  intro n
  by_cases hk : k ≤ n
  · have hsub : ∀ q ∈ (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)).gates.flatMap Gate.wires,
        q ∈ uw n := by
      intro q hq
      obtain ⟨pg, hp, hqp⟩ := List.exists_of_mem_flatMap hq
      have hp' : pg ∈ (VQ.Curve.PointAddition.Arithmetic.Sq.gen n).gates.flatMap VQ.Reversible.compileGate := hp
      obtain ⟨g, hg, hpg⟩ := List.exists_of_mem_flatMap hp'
      have hg' : g ∈ body n := by
        have : (VQ.Curve.PointAddition.Arithmetic.Sq.gen n).gates = if k ≤ n then body n else [] := rfl
        rw [this, if_pos hk] at hg
        exact hg
      exact body_uw hk g hg' q (compileGate_wires hpg hqp)
    have h := dedup_le (2 * k + ws) (uw n)
      ((VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)).gates.flatMap Gate.wires)
      (Nat.le_of_eq (uw_length n)) hsub
    rw [uw_length] at h
    show ((VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)).gates.flatMap Gate.wires).eraseDups.length ≤ 2050
    have e1 : k = 256 := rfl
    have e2 : ws = 1538 := rfl
    omega
  · show ((VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)).gates.flatMap Gate.wires).eraseDups.length ≤ 2050
    have he : (VQ.Curve.PointAddition.Arithmetic.Sq.gen n).gates = [] := by
      have : (VQ.Curve.PointAddition.Arithmetic.Sq.gen n).gates = if k ≤ n then body n else [] := rfl
      rw [this, if_neg hk]
    have : (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)).gates = [] := by
      show (VQ.Curve.PointAddition.Arithmetic.Sq.gen n).gates.flatMap VQ.Reversible.compileGate = []
      rw [he]; rfl
    rw [this]
    simp
...
on basis indices.  The lemmas through `VQ.Curve.PointAddition.Arithmetic.Sq.act_ccx` establish the required
...
/-- The gating routine's action, which is the multiplier's verbatim: the
register at `D` receives the register at `A` when the control is set and nothing
when it is clear.  The degenerate position computes `bv i ctl * bv i ctl`, which is
`bv i ctl`, so the CNOT standing in for the Toffoli there writes the same bit, and
the conclusion needs no case of its own. -/
theorem scopy_act {ctl : Nat} : ∀ (p A D i : Nat), (ctl < D ∨ D + p ≤ ctl) →
    (A + p ≤ D ∨ D + p ≤ A) →
    actGates (scopy ctl A D p) i
  ...
  | succ p ih =>
    ...
    have hhead : RGate.act (if A = ctl then RGate.cx ctl D else RGate.ccx ctl A D) i
        = writeField i D 1 ((bv i D + bv i ctl * bv i A) % 2) := by
      by_cases hAc : A = ctl
      · rw [if_pos hAc]
        have hsq : bv i ctl * bv i A = bv i ctl := by
          rw [hAc]
          rcases ht with ht | ht <;> rw [ht] <;> rfl
        rw [hsq]
        exact act_cx ctl D i
      · rw [if_neg hAc]
        exact act_ccx ctl A D i
...
/-- One round of the product.  Bit `t` of the input gates a copy of the input,
the copy is added into the accumulator at offset `t`, and the copy is cleared by
repeating the same gates: every wire the gating call reads lies in the input
field, which the addition does not touch. -/
theorem mround_act {n : Nat} (hk : k ≤ n) {t i p : Nat} (ht : t < k)
    (hp : p < 2 ^ (k + t)) (hc : bv i (cReg n) = 0) :
    actGates (mround n t) (ms n i 0 p)
      = ms n i 0 (p + bv i t * readField i 0 k * 2 ^ t) := by
  have h1 := e_s n
  have h2 := e_p n
  have h3 := e_v n
  have h4 := e_q n
  ...
  obtain ⟨β, hβ⟩ : ∃ x, bv i t = x := ⟨_, rfl⟩
  have hAk : A < 2 ^ k := hA ▸ readField_lt i 0 k
  have hβ2 : β < 2 := hβ ▸ bv_lt i t
  ...
  have hpdiv : p / 2 ^ t < 2 ^ k := by
    ...
    exact hp
  have hbvB : ∀ s q : Nat, bv (ms n i s q) t = β := by
    intro s q
    rw [← readField_one, ms_out (o := t) (e := 1) (by omega), readField_one, hβ]
  ...
  -- the gated copy of the input
  have step1 : actGates (scopy t 0 (sReg n) k) (ms n i 0 p) = ms n i (β * A) p := by
    rw [scopy_act k 0 (sReg n) (ms n i 0 p) (Or.inl (by omega)) (Or.inl (by omega)),
  ...
  have step3 : actGates (scopy t 0 (sReg n) k) (ms n i (β * A) (p + β * A * 2 ^ t))
      = ms n i 0 (p + β * A * 2 ^ t) := by
    rw [scopy_act k 0 (sReg n) (ms n i (β * A) (p + β * A * 2 ^ t))
    ...
    rw [Nat.zero_div, ms_readS (d := k) (e := 1) (by omega), Nat.div_eq_of_lt hs,
      Nat.zero_mod]
  show actGates ((scopy t 0 (sReg n) k ++ achain (sReg n) (pReg n + t) (cReg n) [] 0 sk)
      ++ scopy t 0 (sReg n) k) (ms n i 0 p) = _
  rw [actGates_append, actGates_append, step1, step2, step3, hβ, hA]

/-- The whole product.  After `t` rounds the accumulator holds the input times
the low `t` bits of the input, and the shifted addend is clear.  This is where the
two operands of the multiplier become one: the multiplier bit for round `t` is
read from the same field the multiplicand is read from, so one hypothesis fixes
both. -/
theorem mloop_act {n : Nat} (hk : k ≤ n) {i A : Nat} (hc : bv i (cReg n) = 0)
    (hA : readField i 0 k = A) : ∀ t, t ≤ k →
    actGates (mloop n t) (ms n i 0 0) = ms n i 0 (A * (A % 2 ^ t)) := by
  have hAk : A < 2 ^ k := hA ▸ readField_lt i 0 k
  intro t
  induction t with
  | zero =>
  ...
  | succ t ih =>
    intro ht
    have hpt : (0 : Nat) < 2 ^ t := Nat.two_pow_pos t
    have hbit : bv i t = A / 2 ^ t % 2 := by
      have h := readField_sub_read (i := i) (off := 0) (N := k) (d := t) (e := 1)
        (show t + 1 ≤ k by omega)
      rw [Nat.zero_add, readField_one, hA, Nat.pow_one] at h
      exact h
    have hmod : A % 2 ^ (t + 1) = A % 2 ^ t + 2 ^ t * (A / 2 ^ t % 2) := by
      rw [Nat.pow_succ, Nat.mod_mul]
    have hlt : A * (A % 2 ^ t) < 2 ^ (k + t) := by
      have h := Nat.mul_lt_mul_of_lt_of_le hAk (Nat.le_of_lt (Nat.mod_lt A hpt)) hpt
      rw [← Nat.pow_add] at h
      exact h
    have harith : A * (A % 2 ^ (t + 1))
        = A * (A % 2 ^ t) + A / 2 ^ t % 2 * A * 2 ^ t := by
      rw [hmod, Nat.mul_add]
      congr 1
      rw [Nat.mul_comm (2 ^ t) (A / 2 ^ t % 2), ← Nat.mul_assoc,
        Nat.mul_comm A (A / 2 ^ t % 2)]
...
theorem gates_le : ∀ n : Nat,
    VQ.Circuit.gateCount (VQ.Reversible.compile (VQ.Curve.PointAddition.Arithmetic.Sq.gen n)) ≤ 8393472 := by
  intro n
  rw [VQ.Reversible.gateCount_compile]
  show (if k ≤ n then body n else []).length
    + 2 * (if k ≤ n then body n else []).countP RGate.isCcx ≤ 8393472
  by_cases hk : k ≤ n
  · rw [if_pos hk]
...
quotient data in the quotient register.  The residue is copied into the output field,
which the forward pass never names, and the reversed forward pass then restores
every workspace wire and the input. -/

theorem squares_general : ∀ n : Nat, VQ.Reversible.SquaresMod VQ.Curve.PointAddition.Arithmetic.Sq.m VQ.Curve.PointAddition.Arithmetic.Sq.ws n (VQ.Curve.PointAddition.Arithmetic.Sq.gen n) := by
  intro n a i _hi h0 h1 h2 hm ha
  have hk : k ≤ n := k_le hm
  have hkp := k_pos
  have h6 := e_sk
  have h7 := e_dk
  ...
  have e1 : readField i n n = 0 := h1
  have e2' : readField i (n + n) ws = 0 := h2
  have e3 : readField i (2 * n) ws = 0 := by
  ...
  have habd : a * a < 2 ^ dk := by
    have h1' : a * a < 2 ^ k * 2 ^ k :=
      Nat.mul_lt_mul_of_lt_of_le (show a < 2 ^ k by omega) (show a ≤ 2 ^ k by omega)
        (Nat.two_pow_pos k)
    rw [hdkkk]
    exact h1'
  have hact : act (VQ.Curve.PointAddition.Arithmetic.Sq.gen n) i = actGates (body n) i := by
    show actGates (if k ≤ n then body n else []) i = _
    rw [if_pos hk]
  -- the product
  have hmul : actGates (mloop n k) i = writeField i (pReg n) dk (a * a) := by
    have h := mloop_act hk wC ea k (Nat.le_refl k)
    rw [ms_id wS wP] at h
    rw [h, Nat.mod_eq_of_lt (show a < 2 ^ k by omega)]
    unfold ms
    rw [show writeField i (sReg n) sk 0 = i from by rw [← wS]; exact writeField_read _ _ _]
  -- the reduction
  have hP1 : readField (writeField i (pReg n) dk (a * a)) (pReg n) dk = a * a := by
    rw [readField_writeField, Nat.mod_eq_of_lt habd]
  have hQ1 : readField (writeField i (pReg n) dk (a * a)) (qReg n) k = 0 := by
    rw [readField_writeField_of_disjoint (Or.inl (by omega))]
    exact wQ
  have hV1 : readField (writeField i (pReg n) dk (a * a)) (vReg n) dk = 0 := by
    rw [readField_writeField_of_disjoint (Or.inl (by omega))]
    exact wV
  have hC1 : bv (writeField i (pReg n) dk (a * a)) (cReg n) = 0 := by
    rw [← readField_one, readField_writeField_of_disjoint (Or.inl (by omega)), readField_one]
    exact wC
  obtain ⟨z, hz⟩ := rloop_act hk k (Nat.le_refl k) _ (a * a) hP1 habm hQ1 hV1 hC1
  have hfwd : actGates (fwd n) i
      = writeField (writeField i (pReg n) dk (a * a % m)) (qReg n) k z := by
  ...
  have hJP : readField J (pReg n) k = a * a % m := by
    rw [← hJ, hfwd, readField_writeField_of_disjoint (Or.inr (by omega))]
    have h := readField_write_sub (x := i) (off := pReg n) (N := dk) (v := a * a % m)
      (d := 0) (e := k) (by omega)
    rw [Nat.add_zero, Nat.pow_zero, Nat.div_one, Nat.mod_eq_of_lt hRk] at h
    exact h
  have hJ2 : readField J n k = 0 := by
    rw [← hJ, hfwd, readField_writeField_of_disjoint (Or.inr (by omega)),
      readField_writeField_of_disjoint (Or.inr (by omega)),
      readField_narrow i n k n hk, e1, Nat.zero_mod]
  have hcopy : actGates (pcopy (pReg n) n k) J = writeField J n k (a * a % m) := by
    rw [pcopy_act k (pReg n) n J (Or.inr (by omega)), hJ2, hJP, Nat.zero_xor]
  have hrev : actGates (fwd n).reverse (writeField J n k (a * a % m))
      = writeField i n k (a * a % m) := by
    rw [actGates_out (fun g hg => fwd_out hk g (List.mem_reverse.mp hg)) J, ← hJ,
      revUndo (fwd_wf hk) i]
  rw [hact]
  show actGates (body n) i = writeField i n n (a * a % m)
  rw [show body n = (fwd n ++ pcopy (pReg n) n k) ++ (fwd n).reverse from rfl,
    actGates_append, actGates_append, hJ, hcopy, hrev]
  exact write_widen hk hRk (by rw [e1]; exact Nat.two_pow_pos k)

/-- Modular squaring with the modulus and workspace specialized to their fixed values. -/
theorem squares : ∀ n : Nat,
    VQ.Reversible.SquaresMod
      115792089237316195423570985008687907853269984665640564039457584007908834671663
      1538 n (VQ.Curve.PointAddition.Arithmetic.Sq.gen n) := squares_general
